...
for i,v in pairs(plrs:GetPlayers()) do
	...
	if string.find(message,lock) then
        game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("bot has been locked to whitelist only","All")
		locked = true
	end
			else
				if plr.Name == 'CJstickguy' then
				if string.find(message, follow) then
            local Args = GetArgs(message)
            local r = FindPlayer(Args[2])
            if r then
	...
	if string.find(message,lock) then
        game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("bot has been unlocked and now everyone can use it","All")
		locked = false
